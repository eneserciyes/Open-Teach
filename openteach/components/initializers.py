    ...
    def _init_camera_processes(self):
        ...
        for camera in self.configs.robot_cam_serial_numbers:
            ...
            self.processes.append(
                Process(target=self._start_component, args=(cam_idx, cam_serial_num))
            )
...
class FishEyeCameras(ProcessInstantiator):
    ...
    def _start_component(self, cam_idx, cam_serial_num):
        print(
            "cam_idx: {}, stream_oculus: {}".format(
                cam_idx, True if self.configs.oculus_cam == cam_idx else False
            )
        )
...
class Collector(ProcessInstantiator):
    ...
    def _init_sim_recorders(self):
        ...
        for key in self.configs.robot.recorded_data[0]:
            self.processes.append(
                Process(target=self._start_sim_component, args=(port_configs[0], key))
            )

    # Function to start the xela sensor recorders
    def _start_xela_component(self, controller_config):
        component = XelaSensorRecorder(
            controller_configs=controller_config, storage_path=self._storage_path
        )
        component.stream()
    ...
    def _init_sensor_recorders(self):
        ...
        for controller_config in self.configs.robot.xela_controllers:
            self.processes.append(
                Process(target=self._start_xela_component, args=(controller_config,))
            )
    ...
    def _start_robot_component(self, robot_configs, recorder_function_key):
